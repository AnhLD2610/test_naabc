...
class Manager(object):
    ...
    def train_model(self, encoder, training_data, seen_des, seen_relations, list_seen_des, is_memory=False):
        ...
        for i in range(epoch):         
            for batch_num, (instance, labels, ind) in enumerate(data_loader):
                ...
                # loss2 = self.moment.mutual_information_loss_cluster(hidden, rep_des, labels, temperature=0.1,relation_2_cluster=relation_2_cluster)  # Recompute loss2
                ...
                if flag == 0:
                    ...
                    loss = loss1 + 1*loss2 + 0.5*loss3 + 0.5*loss4

                    # loss = loss1 + 1*loss2
                    # loss = loss2 + 0.5*loss3 + 0.5*loss4
    ...
    # def train_model_sam(self, encoder, training_data, seen_des, seen_relations, list_seen_des, is_memory=False):
    #     data_loader = get_data_loader_BERT(self.config, training_data, shuffle=True)

    #     base_optimizer = torch.optim.Adam

    #     # optimizer = optim.Adam(params=encoder.parameters(), lr=self.config.lr)
    #     optimizer = SAM(encoder.parameters(), base_optimizer, lr=self.config.lr) 
    ...
    #             loss2 = self.moment.mutual_information_loss(hidden, rep_des, labels)
    ...
    #             # Second forward pass to compute the loss again
    #             hidden = encoder(instance)  # Recompute hidden after first step
    #             rep_des = encoder(batch_instance, is_des=True)  # Recompute rep_des after first step

    #             loss2 = self.moment.mutual_information_loss(hidden, rep_des, labels, temperature=0.05)  # Recompute loss2

    #             if i >= 8:
    #                 # Repeat cluster centroids calculation if necessary
    #                 nearest_cluster_centroids = []
    #                 for hid in hidden:
    #                     cos_similarities = torch.nn.functional.cosine_similarity(hid.unsqueeze(0), cluster_centroids, dim=1)
    ...
    def train(self):
        ...
        for step, (training_data, valid_data, test_data, current_relations, \
            ...
            #     self.train_model(encoder, training_data_initialize, seen_des, seen_relations, list_seen_des, is_memory=True)
