...
class Manager(object):
    ...
    def train_model(self, encoder, training_data, seen_des, seen_relations, list_seen_des, is_memory=False):
        ...
        for i in range(epoch):         
            for batch_num, (instance, labels, ind) in enumerate(data_loader):
                ...
                #     # loss = loss1 + 0.5*loss3 + 0.5*loss4
                #     # loss = 2*loss2 + 0.5*loss3 + 0.5*loss4

                # else:
                loss2 = self.moment.mutual_information_loss_cluster(hidden, rep_des, labels, temperature=0.06)  # Recompute loss2
    ...
    def train(self):
        ...
        for step, (training_data, valid_data, test_data, current_relations, \
            ...
            # if step > 0:
            #     self.train_model_sam(encoder, training_data_initialize, seen_des, seen_relations, list_seen_des, is_memory=False)
